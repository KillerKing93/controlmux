 ...
 * control panel GUI, and runs the main Win32 message pump loop.
 */
...
/**
 * @brief Hidden message window procedure: receives WM_INPUT and WM_TRAYICON.
 *        The visible control panel has its own WndProc (PanelWndProc).
 */
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/,
    ...
    // 1. Load configuration (clear defaults first to avoid double-appending persons)
    ...
    if (!g_renderer->Initialize(hInstance)) {
        MessageBoxW(NULL, L"Failed to initialize GDI+ Overlay Window.",
                    L"ControlMux Error", MB_ICONERROR | MB_TOPMOST);
        return 1;
    }

    // 4. Register hidden message window for Raw Input
    WNDCLASSEXW wc = {};
    ...
    HWND hwndMain = CreateWindowExW(
        0, L"ControlMuxMainWndClass", L"ControlMux",
